...
router.post('/signup', async (req, res) => {
    ...
    if (error) {
        ...
        const hash = await bcrypt.hash(value.password, 10); // operación hash 
        value.password = hash;
        User.create({ name: value.name, email: value.email, password: value.password, sex: value.sex, about: value.about }).then((document) => {
            res.json({ message: true }); // El usuario se ha creado correctamente. 
        }).catch((error) => {
            res.send({ message: error.message });// la creación del usuario falló. 
        });
    }
});
...
router.post('/signin', async (req, res) => {
    User.findOne({ email: req.body.email }).then((document) => {
        if (document) { // Usuario disponible 
